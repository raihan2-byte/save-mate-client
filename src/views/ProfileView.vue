<template>
  <div class="p-4 md:p-8 max-w-3xl mx-auto">
    ...
    <div class="glass-card p-6 flex items-center gap-5 mb-5">
      <div class="w-16 h-16 rounded-2xl bg-gradient-to-br from-emerald-400 to-emerald-600 flex items-center justify-center text-3xl font-black text-white flex-shrink-0"
    ...
    <!-- Budget harian card -->
    <div v-if="budget" class="rounded-2xl border border-emerald-500/20 mb-5 overflow-hidden"
      style="background: linear-gradient(135deg, #064e3b 0%, #065f46 60%, #047857 100%)">
      <div class="p-5">
        <p class="text-emerald-300 text-[10px] font-bold uppercase tracking-widest mb-3">Budget Aktif Bulan Ini</p>
        ...
        <div class="flex justify-between text-xs text-emerald-200/60 px-1">
          <span>Wajib/bulan: {{ fmtShort(budget.total_mandatory ?? 0) }}</span>
          <span>Tabungan/bulan: {{ fmtShort(budget.savings_amount ?? 0) }}</span>
        </div>
      </div>
      <div class="px-5 pb-4">
