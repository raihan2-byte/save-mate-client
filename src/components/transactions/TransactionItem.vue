<template>
  ...
  >
    <div class="flex items-center gap-3">
      ...
      <div>
        <p class="font-medium text-white text-sm">{{ transaction.description || '—' }}</p>
        <p class="text-xs text-slate-500">{{ categoryLabel(transaction.category) }}</p>
      </div>
    </div>
    <div class="flex items-center gap-2">
      <span class="font-bold text-sm tabular-nums text-red-400">-{{ formatCurrency(transaction.amount) }}</span>
      <div class="flex gap-1 opacity-0 group-hover/item:opacity-100 transition-opacity">
        <button
          @click="emit('edit', transaction)"
          class="w-7 h-7 rounded-lg bg-white/5 hover:bg-blue-500/20 flex items-center justify-center text-slate-400 hover:text-blue-400 transition-colors text-xs"
        >✏️</button>
        <button
